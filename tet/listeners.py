...
class BotExtensionTetListeners(BotExtensionTetSubCog):
	...
	async def on_member_update(self,before:Member,after:Member) -> None:
		...
		if PUNISHMENT_ROLE not in [role.id for role in before.roles]: return
		if PUNISHMENT_ROLE in [role.id for role in after.roles]: return
		...
		await channel.send(
			embed=view.embed(before,await self.client.helpers.embed_color(before.guild.id)),
			view=view)
